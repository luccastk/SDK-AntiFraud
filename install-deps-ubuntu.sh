...
print_header "🔧 Instalador de Dependências - SDK AntiFraud Ubuntu"
print_header "=================================================="

# Verificar se está rodando como root
if [ "$EUID" -eq 0 ]; then
    print_warning "Este script não deve ser executado como root"
    print_status "Execute sem sudo: ./install-deps-ubuntu.sh"
    exit 1
fi

# Verificar se apt está disponível
if ! command_exists apt; then
    print_error "apt não encontrado. Este script é para sistemas baseados em Debian/Ubuntu"
    exit 1
fi
